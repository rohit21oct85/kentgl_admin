...
{//print_r($aduser);exit;
	...
	$selUser = "select fstr_email from ffms_users where fstr_email='$email' OR fstr_employee_code='$emp_code'";
	$resUser=mysql_query($selUser);
	$resUsers=mysql_fetch_array($resUser);
	...
	{
		echo '<script language="javascript">alert("This email_id or employee code is already created")</script>';
...
{				if($_SESSION['p_id']=='')
				...
				$all_users = "SELECT fu.*,fr.fstr_role_name FROM ffms_users AS fu LEFT JOIN ffms_role AS fr ON(fu.fsnum_eployee_role=fr.fnum_role_id)";
...
function active_role($r_id,$status)
{
				
			 $query ="UPDATE ffms_role SET fnum_isactive='$status' WHERE fnum_role_id='$r_id'";
		     $result = mysql_query($query);
				if($result)
				{
					echo '<script language="javascript">alert("Record updated sucessfully  ")</script>';
					echo '<script language="javascript">window.location.href = "add_role.php"</script>';
				}
				else
				{
				      echo '<script language="javascript">alert("Record Not updated")</script>';	
...
function attendence_list()
{
				...
				$attend_repo = "select *,(select STR_TO_DATE (fstr_checkin_time , '%h:%i%p') from ffms_users_activity where date(fdt_entry_date)=date(todat.attdate) and fnum_userId=todat.fnum_userId order by fdt_entry_date asc limit 1) checkinTime,(select STR_TO_DATE (fstr_checkout_time , '%h:%i%p') from ffms_users_activity where date(fdt_entry_date)=date(todat.attdate) and fnum_userId=todat.fnum_userId order by fdt_entry_date desc limit 1) checkouttime from (select datas.fnum_userId,date(datas.attdate) as attdate,SEC_TO_TIME( SUM( TIME_TO_SEC((datas.hour)))) hour,datas.employeecode,datas.employeename,datas.city,datas.state,datas.role from (SELECT ua.fnum_userId,ua.fdt_entry_date attdate,TIME_FORMAT(timediff(concat(ua.fstr_checkout_date,' ',STR_TO_DATE (ua.fstr_checkout_time , '%h:%i%p')),concat(ua.fstr_checkin_date,' ',STR_TO_DATE (ua.fstr_checkin_time , '%h:%i%p'))),'%H:%i') as hour,u.fstr_employee_code employeecode,u.fstr_employee_name employeename,u.ffms_city city,u.ffms_state state,r.fstr_role_name role FROM ffms_users_activity AS ua LEFT JOIN ffms_restaurent As fr ON (ua.fnum_restaurent_id = fr.fnum_restaurent_id) LEFT JOIN ffms_users As u ON (ua.fnum_userId = u.fnum_userId) LEFT JOIN ffms_role As r ON (u.fsnum_eployee_role = r.fnum_role_id) WHERE ua.fnum_isactive ='1'"; 
				
				if($admin!='admin')
				{
				$attend_repo .= " AND ua.fnum_userId IN($p_ids)";
				}

				$attend_repo .= " group BY ua.fdt_entry_date) datas group by datas.fnum_userId,date(datas.attdate),datas.employeecode,datas.employeename,datas.role ) todat";
